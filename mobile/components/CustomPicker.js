...
export default function CustomPicker({ value, onValueChange, items = [], style }) {
  ...
  const itemBorderColor = theme === 'dark' ? '#333' : '#eee';

  const selectedLabel = items.find(i => i.value === value)?.label || '';

  return (
    <>
      <TouchableOpacity onPress={() => setOpen(true)} style={[styles.button, { backgroundColor: bg, borderColor: borderColor }, style]}>
        ...
        <TouchableOpacity style={styles.backdrop} onPress={() => setOpen(false)} activeOpacity={1}>
          <View style={[styles.sheet, { backgroundColor: bg }] }>
            <FlatList data={items} keyExtractor={i => String(i.value)} renderItem={({ item }) => (
              <TouchableOpacity onPress={() => { onValueChange(item.value); setOpen(false); }} style={[styles.item, { backgroundColor: itemBg, borderBottomColor: itemBorderColor }] }>
